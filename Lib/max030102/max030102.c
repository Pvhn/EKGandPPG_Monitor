...
int max030201_writeRegMask(const uint8_t regAddr, uint8_t data, uint8_t mask)
{
	...
	HAL_I2C_Mem_Read(&h_I2Cbus, MAX03I2CADDR_R, regAddr, 1, &dataBuffer, 1, 1000);

	// Mask out what we want then write
	dataBuffer = (dataBuffer) & mask;
	dataBuffer = (dataBuffer) | data;
	HAL_I2C_Mem_Write(&h_I2Cbus, MAX03I2CADDR_W, regAddr, 1, &dataBuffer,1, 1000);
...
{
	int error = 0;
	HAL_I2C_Mem_Write(&h_I2Cbus, MAX03I2CADDR_W, regAddr, 1, pData, bytes, 1000);
...
{
	int error = 0;
	HAL_I2C_Mem_Read(&h_I2Cbus, MAX03I2CADDR_W, regAddr, 1, pData, bytes, 1000);
	// TBD ADD ERROR CHECKING
	return error;
}
